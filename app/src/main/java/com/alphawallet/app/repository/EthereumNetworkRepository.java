...
public class EthereumNetworkRepository extends EthereumNetworkBase
{
    ...
    private void buildPopularTokenMap(List<Long> networkFilters)
    {
        ...
        if (networkFilters == null || networkFilters.contains(POODL_MAINNET_ID))  // chaiwala
        {
            for (UnknownToken unknownToken: knownContract.getXDAI())
            {
                popularTokens.put(unknownToken.address.toLowerCase(), new ContractLocator(unknownToken.address, POODL_MAINNET_ID)); // chaiwala
            }
        }
    }
